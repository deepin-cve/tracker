...
func ImportPackage(filename, version string) error {
	if isImportPackage() {
	...
	if handler == nil {
		return fmt.Errorf("No db handler found for version: '%s'", version)
	}

	// clear packages table records
	err := handler.Delete(&db.Package{}).Error
	if err != nil {
		return err
	}
